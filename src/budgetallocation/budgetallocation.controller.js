const BudgetallocationModel = require('./budgetallocation.model');
  ...
  exports.insert = (req, res) => {
        ...
        if(!req.body.organization){
            ...
            BudgetallocationModel.createBudgetallocation(req.body)
                  .then((result) => {
  ...
  exports.list = (req, res ) => {
      ...
      BudgetallocationModel.list(limit, page,req.query)
          .then((result) => {
  ...
  exports.listAll = (req, res ) => {
    if(req.jwt.permissionLevel>0){
        req.query={...req.query,organization:req.jwt.organization}
      }
    /*
        IMPORTANT
        you can put predefined condition here  based on user and role  
     ...
     */
    BudgetallocationModel.listAll(req.query)
        .then((result) => {
            res.status(200).send(result);
        }).catch((err)=>{
            res.status(400).json( {err:err} );
...
exports.listSuggestions = (req, res ) => {
     ...
     */
    BudgetallocationModel.listSuggestions(req.query)
        .then((result) => {
            res.status(200).send(result);
        }).catch((err)=>{
            res.status(400).json( {err:err} );
  ...
  exports.getById = (req, res) => {
     ...
     */
    BudgetallocationModel.findById(req.params.budgetallocationId,filter)
          .then((result) => {
              res.status(200).send(result);
          }).catch((err)=>{
              res.status(400).json( {err:err} );
  ...
  exports.patchById = (req, res) => {
     ...
     */
      BudgetallocationModel.patchBudgetallocation(req.params.budgetallocationId, req.body,filter)
          .then((result) => {
              res.status(204).send({});
          }).catch((err)=>{
              res.status(400).json( {err:err} );
  ...
  exports.removeById = (req, res) => {
     ...
     */
    BudgetallocationModel.removeById(req.params.budgetallocationId,filter)
          .then((result)=>{
              res.status(204).send({});
          }).catch((err)=>{
              res.status(400).json( {err:err} );
