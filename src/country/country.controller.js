const CountryModel = require('./country.model');
  ...
  exports.insert = (req, res) => {
        ...
        req.body.createAt=funcs.getTime()
            CountryModel.createCountry(req.body)
                  .then((result) => {
  ...
  exports.list = (req, res ) => {
      ...
      CountryModel.list(limit, page,req.query)
          .then((result) => {
  ...
  exports.listAll = (req, res ) => {
     ...
     */
    CountryModel.listAll(req.query)
        .then((result) => {
            res.status(200).send(result);
        }).catch((err)=>{
            res.status(400).json( {err:err} );
...
exports.listSuggestions = (req, res ) => {
     ...
     */
    CountryModel.listSuggestions(req.query)
        .then((result) => {
            res.status(200).send(result);
        }).catch((err)=>{
            res.status(400).json( {err:err} );
  ...
  exports.getById = (req, res) => {
     ...
     */
    CountryModel.findById(req.params.countryId,filter)
          .then((result) => {
              res.status(200).send(result);
          }).catch((err)=>{
              res.status(400).json( {err:err} );
  ...
  exports.patchById = (req, res) => {
     ...
     */
      CountryModel.patchCountry(req.params.countryId, req.body,filter)
          .then((result) => {
              res.status(204).send({});
          }).catch((err)=>{
              res.status(400).json( {err:err} );
  ...
  exports.removeById = (req, res) => {
     ...
     */
    CountryModel.removeById(req.params.countryId,filter)
          .then((result)=>{
              res.status(204).send({});
          }).catch((err)=>{
              res.status(400).json( {err:err} );
